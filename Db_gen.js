...
try{
    await db.query
    ("CREATE TABLE IF NOT EXISTS Users (Id INT AUTO_INCREMENT PRIMARY KEY, Nickname VARCHAR(250) UNIQUE,   Email VARCHAR(250), DOB DATE, PhoneNumber VARCHAR(250),   Password VARCHAR(250))");
    await db.query
    ...
    await db.query
    ("CREATE TABLE IF NOT EXISTS Group_Activity( Id_Activitate INT AUTO_INCREMENT PRIMARY KEY,  Id_Group INT, Id_Membru INT,    Nume VARCHAR(250), Status VARCHAR(250) NOT NULL, Deadline DATE, Acceptat BOOLEAN, FOREIGN KEY(Id_Group) REFERENCES Members(Id_Group), FOREIGN KEY (Id_Membru) REFERENCES Members(Id_Membru)   )")
    await db.query
