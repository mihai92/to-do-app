...
router.post("/invite/:GroupId/:Nickname",Autentificare,async (req,res)=>{
    try{
        ...
        const [query]=await db.execute("INSERT INTO Notifications(Id_Emitator,Id_Group,Id_Interceptor ,Mesaj, Acceptat, Vizibilitate_Acceptare, Notificare_noua) VALUES(?,?,?,?,false,true,true)",[req.auth.id, req.params.GroupId, queryDupaNickname[0].Id,string]);
...
router.put('/inviteResponse/:InviteId',Autentificare, async(req,res)=>{
    ...
    try{
        if(bool===false){
            ...
            const [query5]=await db.execute("INSERT INTO Notifications(Id_Emitator,Id_Group,Id_Interceptor ,Mesaj, Acceptat, Vizibilitate_Acceptare, Notificare_noua) VALUES(?,?,?,?,false,false,true)",[req.auth.id,query3[0].Id_Group,query3[0].Id_Admin,"Un nou membru a venit in grupul tau"]);
    }
